 ...
 */
function F1Register(f1, people, households, statuses) {
...
F1Register.prototype.createPerson = function(reg, callback) {
  // step 1: get the member status block
  this.statuses.list(function(err, statusList) {
    ...
    async.filter(statusList, function(item, callback) {
      ...
      this.households.create(household, function(err, household) {
        if (err) return callback(err)

        person['@householdID'] = household['@id']
        this.people.create(person, callback)
      }.bind(this))
    }.bind(this))
  }.bind(this))
}
