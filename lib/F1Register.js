          ...
          this.communicationTypes.list(function(err, commTypeList) {
            ...
            async.filter(commTypeList, function(item, callback) {
              ...
              comms.create(email, function(err, commsBody) {
                ...
                this.createAddress(reg, body.person, function(err, addr) {
                  callback(err, body)
                }.bind(this))
              }.bind(this))
            }.bind(this))
...
F1Register.prototype.getAddressType = function(type, callback) {
  var PRIMARY = 'Primary'
  var addressTypes = new F1.AddressTypes(this.f1)
  addressTypes.list(function(err, typeList) {
    if (err) return callback(err)

    async.filter(typeList, function(item, callback) {
      callback(item.name == PRIMARY)
    }, function(items) {
      if (_.isEmpty(items)) return callback('No AddressType named `' + PRIMARY + '` could be found!')
...
F1Register.prototype.createAddress = function(reg, person, callback) {
