...
F1Register.prototype._createPhone = function(phoneType, person, reg, callback) {
  if (!!reg[phoneType]) {
    this.getCommunicationType(phoneType, function(err, commType) {
      if (err) return callback(err)
      ...
      var phone = {
        ...
        communicationValue: reg[phoneType],
        searchCommunicationValue: reg[phoneType],
        preferred: 'true'
      }

      var comms = new F1.PersonCommunications(this.f1, person['@id'])
      comms.create(phone, callback)
    }.bind(this))
  } else {
    callback(null, {})
  }
...
F1Register.prototype.createHomePhone = function(person, reg, callback) {
  return this._createPhone(HOME_PHONE, person, reg, callback)
}

F1Register.prototype.createMobilePhone = function(person, reg, callback) {
  return this._createPhone(MOBILE_PHONE, person, reg, callback)
}

F1Register.prototype.createEmail = function(person, reg, callback) {
  this.getCommunicationType(EMAIL, function(err, commType) {
    if (err) return callback(err)
...
F1Register.prototype.getResourceByName = function(pluralResourceName, name, callback) {
  ...
  resources.list(function(err, list) {
    ...
    async.filter(list, function(item, callback) {
      callback(item.name == name)
    }, function(items) {
      if (_.isEmpty(items)) return callback('No ' + pluralResourceName + ' with name `' +
        name + '` could be found!')
