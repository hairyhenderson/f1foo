...
F1Register.prototype.createHomePhone = function(person, reg, callback) {
  if (!!reg['Home Phone']) {
    this.getCommunicationType(HOME_PHONE, function(err, commType) {
      if (err) return callback(err)
      ...
      var phone = {
        ...
        communicationValue: reg['Home Phone'],
        searchCommunicationValue: reg['Home Phone'],
        preferred: 'true'
      }
...
F1Register.prototype.createMobilePhone = function(person, reg, callback) {
  if (!!reg['Mobile Phone']) {
    this.getCommunicationType(MOBILE_PHONE, function(err, commType) {
      if (err) return callback(err)
      ...
      var phone = {
        ...
        communicationType: commType,
        communicationGeneralType: 'Telephone',
        communicationValue: reg['Mobile Phone'],
        searchCommunicationValue: reg['Mobile Phone'],
        preferred: 'true'
      }
